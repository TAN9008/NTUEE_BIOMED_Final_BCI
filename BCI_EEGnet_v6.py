...
def detect_blinks_blinker(
    ...
    """
    簡單版「Blinker」眨眼偵測：
        1) 以 median / MAD 做 robust normalization
        2) 用 20 ms moving average 平滑
        3) 找 |signal| > threshold_mad 的連續區段
        4) 長度在 [min_duration, max_duration] 秒內者視為一個 blink

    回傳值：
        blink_count: 在這段 segment 中偵測到的眨眼數量
    """
    ...
    # robust normalization：減掉 median、除以 MAD
    x = x - np.median(x)
    mad = np.median(np.abs(x)) + 1e-6
    x_norm = x / mad

    # 約 20 ms moving average 平滑
    ...
    # 門檻：|x| > threshold_mad 視為「候選 blink 區域」
...
def train_final_model_and_save(save_path="eegnet_bci.pth"):
    """
    使用所有 subject 的 segment 重新訓練一次 EEGNet，
    並存成給線上 BCI 使用的 checkpoint 檔案。

    存檔格式：
        {
            "state_dict": model.state_dict(),
            "seg_len": segment_length_samples,
            "num_classes": num_classes
        }

    BCI_main_Inter.py 的 init_eegnet_model 會用這個格式載入。
    """
    print("\n[INFO] Training final EEGNet model on ALL subjects...")

    X, y, subjects = load_all_subjects()
    if X is None or y is None:
        print("[ERROR] Cannot load dataset for final model training, skip saving.")
        return
    ...
    print(f"[INFO] Final EEGNet model saved to '{save_path}' (seg_len={seg_len}, num_classes={clf.num_classes})")
...
def main():
    ...
    train_final_model_and_save("eegnet_bci.pth")
