...
BLINK_REFRACTORY_SECONDS = 3.0              # 兩次 blink 之間至少隔這麼久才再觸發
...
def main():
    ...
    def handle_raw(sample: int):
        nonlocal last_save_time, last_blink_time
        buffer.append(sample)

        # --- 1) 每秒寫一段 4 秒的 EEG 到 latest_eeg.npy（給 EEGNet 用） ---
        now = time.time()
        if len(buffer) >= WINDOW_SIZE and (now - last_save_time) >= 1.0:
            ...
            try:
                # 先寫到暫存檔（用檔案物件避免 np.save 自動改檔名）
                with open(tmp_path, "wb") as f:
                    np.save(f, seg)

                # 再用 os.replace 原子性覆蓋 latest_eeg.npy
                os.replace(tmp_path, LATEST_SEGMENT_PATH)
                last_save_time = now
                # print(f"[INFO] saved {len(seg)} samples to {LATEST_SEGMENT_PATH}")
        ...
        # --- 2) Blinker：在最近 2 秒的資料上偵測眨眼，若有就寫入時間戳 ---
        if len(buffer) >= BLINK_DETECT_WINDOW_SIZE:
            seg2 = np.array(list(buffer)[-BLINK_DETECT_WINDOW_SIZE:], dtype=np.float32)
