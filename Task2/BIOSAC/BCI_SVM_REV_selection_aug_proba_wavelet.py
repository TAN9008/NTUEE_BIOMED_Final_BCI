...
Features: band powers (delta/theta/alpha/beta_low/beta_high/gamma),
          ...
          zero-crossing rate (ZCR), wavelet energy (pywt).
...
import glob
import warnings
import pickle  # NEW: for saving the trained model
...
class Config:
    ...
    # 現在總特徵約 39 維（原本 35 + 4 個 wavelet energy）
...
def wavelet_energy(segment, wavelet="db4", level=4):
    ...
    回傳固定長度的 energy 向量（level 維），不足的 level 以 0 填補。
    """
    ...
    # 只用 detail coefficients 的 energy：cD_L, ..., cD_1
    for i in range(actual_level):
        cD = coeffs[i + 1]  # coeffs[1] = cD_L
        e = np.sum(cD ** 2)
...
def extract_features(segment, fs=Config.SAMPLING_RATE):
    ...
    wavelet_feats = wavelet_energy(segment, wavelet="db4", level=4)  # 4 維
    ...
    # 13 (time/HFD/SampEn/ZCR) + 1 (entropy) + 21 (band) + 4 (wavelet) = 39 維
    return feats
...
class BandFeatureSVMClassifier:
    """BCI classifier using SVM with extended band / time / wavelet features."""
    ...
    def __init__(self):
        ...
        num_features = 39  # extract_features 回傳的維度（含 wavelet energy）
...
def leave_one_subject_out_validation():
    print("Starting LOSO Cross-Validation with band + wavelet-feature SVM...")
...
def train_and_save_full_model(model_path="bci_svm.pkl"):
    """
    使用所有 subject 的資料訓練一個最終 SVM 模型，並存成 pickle 檔。
    模型內含：
        - StandardScaler
        - SelectKBest (若有啟用 FEATURE_SELECTION)
        - 已訓練好的 SVC
    之後在其他程式可以直接：
        with open("bci_svm.pkl", "rb") as f:
            clf = pickle.load(f)
    然後呼叫 clf.predict(...) / clf.predict_proba(...)
    """
    print(f"\n[Train & Save] Training final model on ALL subjects and saving to '{model_path}'")

    X, y, subjects = load_all_subjects()
    if X is None:
        print("[Train & Save] Error: cannot train final model because data loading failed.")
        return

    # 可選：在全體訓練資料上做 augmentation
    X_train, y_train = augment_features(X, y)
    ...
    try:
        with open(model_path, "wb") as f:
            pickle.dump(clf, f)
        print(f"[Train & Save] Model saved successfully to '{model_path}'.")
    except Exception as e:
        print(f"[Train & Save] Failed to save model: {e}")
...
def plot_results(results):
    ...
    fig.suptitle("BCI Classifier (Band + Wavelet Features, SVM) - LOSO Results", fontsize=16)
...
def main():
    print("BCI EEG Classification (Band + Wavelet Features + SVM)")
    print("=" * 60)
    ...
    # 2) 再用全部 subject 的資料訓練最終模型並存成 bci_svm.pkl
    train_and_save_full_model("bci_svm.pkl")
